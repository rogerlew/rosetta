...
data_in = np.genfromtxt(args.input, delimiter='', dtype=np.float).transpose()
print(data_in)
# data_in=data_in[0:3,]
# print(data_in)
# print(type(data_in))
...
if args.predict:
    ...
    with DB(host=args.host, user=args.user, db_name=args.db, sqlite_path=args.sqlite) as db:
        ...
        # data=np.array([[90,5,5],[1,1,98],[1,2,97],[2,3,95]],dtype=np.float).transpose()
        # data=np.array([[90,5,5,1.4],[1,1,98,1.0],[2,5,93,1.2]],dtype=np.float).transpose()
        ...
        # np.seterr(over='raise') # debug or new model
        ...
        if not args.raw:
            ...
            vgm_new = np.stack(
                (vgm_mean[0], vgm_mean[1], 10**vgm_mean[2], 10**vgm_mean[3], 10**vgm_mean[4]))
...
if args.check:
    ...
    with DB(host=args.host, user=args.user, db_name=args.db, sqlite_path=args.sqlite) as db:
        ...
        data = np.array([[90, 5, 5], [40, 35, 25]], dtype=np.float).transpose()
        res_dict = ptf_model.predict(data, sum_data=True)
        ...
        data = np.array(
            [[90, 5, 5, 1.5, 0.1], [40, 35, 25, 1.5, 0.2]], dtype=np.float).transpose()
        res_dict = ptf_model.predict(data, sum_data=True)
        print(res_dict['var_names'])
        print("MEAN")
        print(res_dict['sum_res_mean'])
        ...
        data = np.array([[90, 5, 5, 1.5, 0.1, 0.05], [
                        40, 35, 25, 1.5, 0.2, 0.05]], dtype=np.float).transpose()
        res_dict = ptf_model.predict(data, sum_data=True)
        print(res_dict['var_names'])
        print("MEAN")
        print(res_dict['sum_res_mean'])
